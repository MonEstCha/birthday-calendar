...
public:
    ...
    bool IsConnectedToDB() const { return m_connected_to_db; }
...
bool MyApp::OnInit()
{
    ...
    MyFrame *frame = new MyFrame("Birthday Calendar"
                                 ,wxPoint(50, 50), wxSize(460, 340)
                                 );

    frame->Show(true);

    m_connected_to_db = ma::establish_database_connection();
...
void MyFrame::OnAddBirthday(wxCommandEvent& WXUNUSED(event))
{
    wxDateTime dt = m_panel->GetCal()->GetDate();
    ...
    std::string name = "";
    std::string date = "";
    MyAddBirthdayDialog dlg(this, dt, style);
...
{
    wxString dateInfo = "Selected date: " + event.GetDate().FormatISODate();
    wxString s = dateInfo + "\nBirthday of:";


    auto bd = ma::get_birthday(event.GetDate().FormatISODate().ToStdString());
    for(auto name : bd){
        s += ("\n" + (wxString(name)));
    }
    s.Printf(s);
    wxLogStatus(dateInfo);
    m_date->SetLabel(s);
}
...
void MyAddBirthdayDialog::OnBirthdayInsert()
{
    cout << "Birthday insert" << endl;
}
